...
cores = [1, 2]
runtimes = [2176.37, 964.31]

t1 = runtimes[0]
speedups  = [t1 / t for t in runtimes]
ideal = [float(c) for c in cores]
...
# Plot 1: Runtime vs Cores
fig1, ax1 = plt.subplots(figsize=(7, 4.5))
ax1.plot(cores, runtimes, color=BLUE_DARK, marker="o", markersize=8,
...
ax1.set_xlabel("Number of Cores (1 Worker Node)", fontsize=12, color=GREY)
ax1.set_ylabel("Runtime (seconds)", fontsize=12, color=GREY)
ax1.set_title("Runtime vs. Number of Cores\n(Vertical Compute Scaling — 1 Worker Node)",
...
fig1.tight_layout()
fig1.savefig("plot_vertical_compute_runtime.png", bbox_inches="tight")
print("Saved: plot_vertical_compute_runtime.png")

# Plot 2: Speedup vs Ideal
fig2, ax2 = plt.subplots(figsize=(7, 4.5))
ax2.plot(cores, ideal, color=RED_IDEAL, linestyle="--", linewidth=2,
         marker="s", markersize=7, label="Ideal (linear) speedup")
ax2.plot(cores, speedups, color=BLUE_DARK, marker="o", markersize=8,
         linewidth=2.5, label="Measured speedup")
for x, y in zip(cores, speedups):
    ax2.annotate(f"{y:.2f}x", xy=(x, y), xytext=(0, 12),
                 textcoords="offset points", ha="center",
                 fontsize=11, color=BLUE_DARK, fontweight="bold")
ax2.fill_between(cores, ideal, speedups, alpha=0.15, color="#70AD47", label="Super-linear gain")
ax2.set_xlabel("Number of Cores (1 Worker Node)", fontsize=12, color=GREY)
...
fig2.tight_layout()
fig2.savefig("plot_vertical_compute_speedup.png", bbox_inches="tight")
print("Saved: plot_vertical_compute_speedup.png")
